...
class LinearSearch(Experiment):
    def __init__(self):
        super().__init__(name="Linear Search", desc="performing linear search", topics_split=5)
        self.values = {
            'frequency_penalty': {'fixed_value': 0, 'bounds': [-1, 2], 'step': 0.2, "short_name": "fp"},
            'presence_penalty': {'fixed_value': 0, 'bounds': [-1, 2], 'step': 0.2, "short_name": "pp"},
            'temperature': {'fixed_value': 1, 'bounds': [0, 1.5], 'step': 0.1, "short_name": "temp"},
            'top_p': {'fixed_value': 1, 'bounds': [0, 1], 'step': 0.1, "short_name": "topp"}
        }
        self.tokens = 800
