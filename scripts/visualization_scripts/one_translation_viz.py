...
df = pd.read_csv("../../data/One translation/results/result.csv")
save_folder = "..\..\data\One translation\\results"


def extract_mutation(text_id):
    ids = text_id.split("_")[0]
    return ids
...
for detector in unique_detectors:
    ...
    plt.title(f'Average Score by original language for Detector: {detector}')
...
for detector in unique_detectors:
    ...
    sns.boxplot(x='mutation_name', y='score', data=df[df['detector'] == detector],  medianprops={'color': 'red', 'linewidth': 2})
    plt.title(f'Score Distribution by original language for Detector: {detector}')
    plt.xlabel('Original Language')
    plt.ylabel('Score')
